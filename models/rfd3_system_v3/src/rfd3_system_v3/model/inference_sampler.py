...
from rfd3_system_v3.inference.symmetry.symmetry_utils import apply_symmetry_to_xyz_atomwise
from rfd3_system_v3.model.cfg_utils import strip_X
...
class SampleDiffusionConfig:
    ...
    s_max: int = 160
    p: int = 7
    # Legacy sampler knobs are retained only so checkpoint/config merging gives
    # clear validation errors.  The strict SDE paths require these neutral values.
    gamma_0: float = 0.0
    gamma_min: float = 0.0
...
class SampleDiffusionWithMotif(SampleDiffusionConfig):
    ...
    def _noise_schedule_to_normalized_t(
        ...
        normalized_t = (numerator / denominator).clamp(0.0, 1.0)
        return torch.where(
            normalized_t.abs() < 1e-7,
            torch.zeros_like(normalized_t),
            normalized_t,
        )
...
# Import after defining the base class to keep the strict SDE rollout code in a
# focused module without duplicating RFD3's schedule and denoiser helpers.
from rfd3_system_v3.model.sde_sampler import (  # noqa: E402
...
class ConditionalDiffusionSampler:
    """Select one of the two rfd3_system_v3 reverse-SDE samplers."""

    _registry = {
        "reverse_sde": SampleDiffusionReverseSDE,
        "superdiff_shared_chain_sde": SampleDiffusionWithSuperDiffSharedChainSDE,
    }

    def __init__(self, kind="reverse_sde", **kwargs):
        ranked_logger.info(
            f"Initializing ConditionalDiffusionSampler with kind: {kind}"
        )
        try:
            sampler_class = self._registry[kind]
        except KeyError as exc:
            raise ValueError(
                f"Invalid sampler kind: {kind}, must be one of "
                f"{list(self._registry.keys())}"
            ) from exc
        init_args = self.get_class_init_args(sampler_class)
        filtered_kwargs = {
            key: value for key, value in kwargs.items() if key in init_args
        }
        self.sampler = sampler_class(**filtered_kwargs)

    def sample_diffusion_like_af3(self, **kwargs):
        return self.sampler.sample_diffusion_like_af3(**kwargs)

    def sample_coupled_superdiff_sde(self, **kwargs):
        if not hasattr(self.sampler, "sample_coupled_superdiff_sde"):
            raise ValueError(
                "Stochastic shared-chain coupling requires "
                "inference_sampler.kind='superdiff_shared_chain_sde'."
            )
        return self.sampler.sample_coupled_superdiff_sde(**kwargs)
    ...
    def get_class_init_args(cls):
        ...
        if hasattr(cls, "__init__") and callable(cls.__init__):
            for parent in cls.__mro__:
                if "__init__" in parent.__dict__ and parent is not object:
                    signature = inspect.signature(parent.__init__)
                    arg_names.extend(
                        parameter.name
