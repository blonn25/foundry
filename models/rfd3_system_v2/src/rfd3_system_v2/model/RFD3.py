...
from rfd3_system_v2.model.cfg_utils import (
    strip_f,
)
from rfd3_system_v2.model.inference_sampler import ConditionalDiffusionSampler
from rfd3_system_v2.model.layers.encoders import TokenInitializer
...
class RFD3(nn.Module):
    ...
    def forward_coupled(
        ...
        *,
        ...
        """Run deterministic density-controlled inference for two tracks.

        This path is inference-only and intentionally bypasses training loss
        machinery.  It uses the same token initializer and diffusion module as
        ordinary RFD3, but delegates rollout to the
        `superdiff_shared_chain_density` sampler.
        """
        ...
        if self.use_classifier_free_guidance:
            raise ValueError(
                "Density-controlled shared-chain coupling does not support native "
        ...
        return self.inference_sampler.sample_coupled_superdiff_density(
            track_1={
                "f": track_1_input["f"],
