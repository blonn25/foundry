#!/usr/bin/env python3
"""Plot rfd3_system_v3 reverse-SDE and stochastic-coupling diagnostics."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Create PNG plots for rfd3_system_v3 kappa, Itô residual, field "
            "cosine, and drift/noise diagnostics after inference."
        ),
    )
    parser.add_argument(
        "paths",
        nargs="+",
        type=Path,
        help="Output directories or specific rfd3_system_v3 JSON metadata files.",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
...
def _collect_diagnostic_sources(
    ...
    rfd3_system_v3 writes identical diagnostics into track and merged JSON
...
def _diagnostic_source_from_json(json_path: Path) -> DiagnosticSource | None:
    """Return a source if the JSON contains SDE sampling diagnostics."""

    metadata = json.loads(json_path.read_text())
    diagnostic_metadata = metadata.get("coupling") or metadata.get("sampling") or {}
    diagnostics = diagnostic_metadata.get("diagnostics", {})
    if not diagnostics:
        return None
...
def _source_rank(source: DiagnosticSource) -> int:
    ...
    output_kind = str(source.metadata.get("output", ""))
...
def _plot_one_source(
    ...
    *,
    ...
    """Create reverse-SDE diagnostic PNGs for one diffusion batch."""

    coupling = source.metadata
    ...
    if "kappa" in diagnostics:
        ...
        _plot_kappa(
            ...
            kappa_min=_finite_bound(coupling.get("kappa_min"), fallback=-1.0),
            kappa_max=_finite_bound(coupling.get("kappa_max"), fallback=2.0),
            sample_labels=labels,
            dpi=dpi,
        )
        paths.append(kappa_path)

    if "density_residual" in diagnostics:
        residual, labels = _diagnostic_array_for_model(
            diagnostics["density_residual"],
            "density_residual",
            model_index,
        )
        normalized_t = _normalized_t_axis(
        ...
        residual_path = plot_prefix.with_name(
            f"{plot_prefix.name}_ito_density_residual.png"
        )
        _plot_density_residual(
            residual_path,
            normalized_t=normalized_t,
            residual=residual,
            sample_labels=labels,
    ...
    cosine_specs = (
        (
            ...
            "Kappa-subset conditional-field/mixed-field cosine similarity",
        ),
        (
            "all_shared",
            "cosine_v1_mixed_all_shared",
            "cosine_v2_mixed_all_shared",
            "All-shared-atom conditional-field/mixed-field cosine similarity",
        ),
    )
    ...
    if "drift_A_norm" in diagnostics and "noise_A_norm" in diagnostics:
        drift, labels = _diagnostic_array_for_model(
            diagnostics["drift_A_norm"], "drift_A_norm", model_index
        )
        noise, noise_labels = _diagnostic_array_for_model(
            diagnostics["noise_A_norm"], "noise_A_norm", model_index
        )
        if drift.shape != noise.shape or labels != noise_labels:
            raise ValueError("Shared-A drift and noise diagnostics do not align.")
        normalized_t = _normalized_t_axis(
            diagnostics.get("normalized_t"), drift.shape[0]
        )
        path = plot_prefix.with_name(f"{plot_prefix.name}_drift_noise.png")
        _plot_drift_noise(
            path,
            normalized_t=normalized_t,
            drift=drift,
        ...
        paths.append(path)
    elif "drift_norm" in diagnostics and "noise_norm" in diagnostics:
        drift, labels = _diagnostic_array_for_model(
            diagnostics["drift_norm"], "drift_norm", model_index
        )
        noise, noise_labels = _diagnostic_array_for_model(
            diagnostics["noise_norm"], "noise_norm", model_index
        )
        if drift.shape != noise.shape or labels != noise_labels:
            raise ValueError("Single-track drift and noise diagnostics do not align.")
        normalized_t = _normalized_t_axis(
            diagnostics.get("normalized_t"), drift.shape[0]
        )
        path = plot_prefix.with_name(f"{plot_prefix.name}_drift_noise.png")
...
def _finite_bound(value, *, fallback: float) -> float:
    """Use a plotting fallback when a kappa clamp is disabled with null."""
...
def _plot_kappa(
    ...
    *,
    ...
    ax.set_ylabel("kappa in shared-A path field")
    ax.set_xlim(0.0, 1.0)
...
def _plot_density_residual(
    ...
    *,
    ...
    """Plot post-clamp stochastic Itô residual trajectories."""

    fig, ax = plt.subplots(figsize=(8.5, 5.0), constrained_layout=True)
    _plot_samples(ax, normalized_t, residual, sample_labels)
    ax.axhline(0.0, color="0.35", linestyle="--", linewidth=1.0)
    ax.set_title("Post-clamp Itô density-change residual over denoising")
    ax.set_xlabel("Normalized denoising progress t (0 = noisiest, 1 = final)")
    ax.set_ylabel("delta log q1 - delta log q2 - target")
    ax.set_xlim(0.0, 1.0)
    ...
    ax.text(
        ...
        -0.22,
        (
            "Residual near zero means the finite-step conditional density-change "
            "constraint was better satisfied."
        ),
        transform=ax.transAxes,
        fontsize=8,
    )
...
def _plot_cosine_similarity(
    ...
    *,
    ...
    """Plot paired conditional-field/path cosines with one hue per sample."""
    ...
    for sample_idx in range(n_plot):
        ...
        ax.plot(
            ...
            color=light_color,
            linewidth=1.8,
            label=f"{label}: cos(v1, mixed field)",
        )
        ax.plot(
            normalized_t,
            cosine_2[:, sample_idx],
            color=dark_color,
            linewidth=1.8,
            label=f"{label}: cos(v2, mixed field)",
        )
    ...
    note = (
        f"Light shade: cos(v1 from {track_1_label}, mixed field). "
        f"Dark shade: cos(v2 from {track_2_label}, mixed field). "
...
def _plot_drift_noise(
    ...
    *,
    ...
    """Plot deterministic drift and Brownian increment norms together."""

    fig, ax = plt.subplots(figsize=(8.5, 5.0), constrained_layout=True)
    n_samples = drift.shape[1]
    n_plot = min(n_samples, max_samples)
    cmap = plt.get_cmap("tab10")
    for sample_idx in range(n_plot):
        ...
        ax.plot(
            normalized_t,
            drift[:, sample_idx],
            color=_blend_with_black(base_color, 0.18),
            linewidth=1.8,
            label=f"{label}: drift norm",
        )
        ax.plot(
            normalized_t,
            noise[:, sample_idx],
            color=_blend_with_white(base_color, 0.62),
            linewidth=1.8,
            label=f"{label}: Brownian-noise norm",
        )
    ax.set_title("Reverse-SDE drift and Brownian noise over denoising")
    ax.set_xlabel("Normalized denoising progress t (0 = noisiest, 1 = final)")
    ax.set_ylabel("Coordinate-update norm")
    ax.set_xlim(0.0, 1.0)
