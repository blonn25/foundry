...
from rfd3_system_v3.constants import SAVED_CONDITIONING_ANNOTATIONS
from rfd3_system_v3.inference.datasets import (
    assemble_distributed_inference_loader_from_json,
)
from rfd3_system_v3.inference.input_parsing import (
    DesignInputSpecification,
    ensure_input_is_abspath,
    prepare_pipeline_input_from_atom_array,
)
from rfd3_system_v3.model.inference_sampler import SampleDiffusionConfig
...
from rfd3_system_v3.utils.inference import (
    ensure_inference_sampler_matches_design_spec,
)
from rfd3_system_v3.utils.io import (
    CIF_LIKE_EXTENSIONS,
...
class RFD3InferenceConfig:
    ...
    # Stochastic reverse-SDE shared-chain coupling. Off by default.
    ...
    unconditional_reference_track: Optional[Literal["track1", "track2"]] = None
    merged_output_policy: Literal["track1", "track2", "both", "none"] = "both"
...
class RFD3InferenceEngine(BaseInferenceEngine):
    ...
    def __init__(
        ...
        *,
        ...
        unconditional_reference_track: str | None = None,
        merged_output_policy: str = "both",
        **kwargs,
    ):
        super().__init__(
        ...
        # Coupled sampling configuration. These values are engine-level
        ...
        self.unconditional_reference_track = unconditional_reference_track
    ...
    def _override_checkpoint_config(self, cfg):
        """Load the RFD3 checkpoint config through the rfd3_system_v3 package.

        The public RFD3 checkpoint stores Hydra targets under the original
        `rfd3.*` package. This research copy must instantiate local
        `rfd3_system_v3.*` classes so the coupled sampler registry and engine
        ...
        if isinstance(node, DictConfig):
            if "_target_" in node and isinstance(node["_target_"], str):
                ...
                if target.startswith("rfd3."):
                    node["_target_"] = "rfd3_system_v3." + target[len("rfd3.") :]
    ...
    def run(
        ...
        *,
        ...
        if len(design_specifications) == 0:
            ...
            return None
        if self.coupling_mode == "superdiff_shared_chain_sde":
            if (
                self.inference_sampler_overrides.get("kind")
                != "superdiff_shared_chain_sde"
            ):
                raise ValueError(
                    "coupling_mode='superdiff_shared_chain_sde' requires "
                    "inference_sampler.kind='superdiff_shared_chain_sde'."
                )
            guidance_scale = float(
                self.inference_sampler_overrides.get(
                    "superdiff_guidance_scale", 1.0
                )
            )
            if guidance_scale != 1.0 and self.unconditional_reference_track not in (
                "track1",
                "track2",
            ):
                raise ValueError(
                    "unconditional_reference_track must be set to 'track1' or "
                    "'track2' when superdiff_guidance_scale != 1."
                )
            self.initialize()
    ...
    def _model_forward(self, pipeline_output) -> List[RFD3Output]:
        ...
        outputs = []
        sampling_metadata = output_val["network_output"].get("sampling_metadata")
        for idx in range(len(output_val["predicted_atom_array_stack"])):
            if self.dump_prediction_metadata_json:
                ...
                if sampling_metadata is not None:
                    output_val["prediction_metadata"][idx]["sampling"] = _to_jsonable(
                        sampling_metadata
                    )
    ...
    def _run_superdiff_shared_chain(
        ...
        """Run stochastic reverse-SDE SuperDiff for A+B and A+C."""

        if self.trainer.fabric.world_size != 1:
            raise ValueError(
                "Stochastic shared-chain coupling supports one process. "
    ...
    def _model_forward_coupled(
        ...
        track_1_output, track_1_raw, track_1_spec_dict = (
            self._build_spec_pipeline_output(
                track_1_spec,
                example_id=f"{example_id}_track1",
            )
        )
        track_2_output, track_2_raw, track_2_spec_dict = (
            self._build_spec_pipeline_output(
                track_2_spec,
                example_id=f"{example_id}_track2",
            )
        )
        ...
        shared_token_indices_1_np, shared_token_indices_2_np = (
            shared_update_token_indices(
                shared_atom_map,
                self._feature_vector_np(track_1_output, "atom_to_token_map"),
                self._feature_vector_np(track_2_output, "atom_to_token_map"),
                self._feature_mask_np(
                    track_1_output, "is_motif_atom_with_fixed_seq"
                ),
                self._feature_mask_np(
        ...
        guidance_scale = float(
            self.inference_sampler_overrides.get("superdiff_guidance_scale", 1.0)
        )
        reference_output = None
        reference_atom_map = None
        reference_kappa_indices_np = None
        reference_token_indices_np = None
        if guidance_scale != 1.0:
            if self.unconditional_reference_track == "track1":
                selected_output = track_1_output
                selected_raw = track_1_raw
                selected_spec_dict = track_1_spec_dict
                selected_update_indices = shared_atom_map.update_indices_1
                selected_kappa_indices = kappa_indices_1_np
            else:
                selected_output = track_2_output
                selected_raw = track_2_raw
                selected_spec_dict = track_2_spec_dict
                selected_update_indices = shared_atom_map.update_indices_2
                selected_kappa_indices = kappa_indices_2_np

            reference_raw = subset_by_chains(
                selected_raw,
                [self.shared_chain_id],
            )
            reference_output = self._build_pipeline_output_from_atom_array(
                reference_raw,
                selected_spec_dict,
                example_id=f"{example_id}_shared_reference",
            )
            reference_atom_map = build_shared_update_atom_map(
                selected_output["atom_array"],
                reference_output["atom_array"],
                self.shared_chain_id,
                self._feature_mask_np(
                    selected_output, "is_motif_atom_with_fixed_coord"
                ),
                self._feature_mask_np(
                    reference_output, "is_motif_atom_with_fixed_coord"
                ),
                self._feature_mask_np(
                    selected_output, "is_motif_atom_with_fixed_seq"
                ),
                self._feature_mask_np(
                    reference_output, "is_motif_atom_with_fixed_seq"
                ),
            )
            selected_kappa_check, reference_kappa_indices_np = (
                select_kappa_solve_atom_indices(
                    reference_atom_map,
                    selected_output["atom_array"],
                    reference_output["atom_array"],
            ...
            if not np.array_equal(
                reference_atom_map.update_indices_1,
                selected_update_indices,
            ) or not np.array_equal(selected_kappa_check, selected_kappa_indices):
                raise ValueError(
                    "The isolated shared-chain reference does not preserve the "
                    "selected track's shared atom ordering."
                )
            selected_token_check, reference_token_indices_np = (
                shared_update_token_indices(
                    reference_atom_map,
                    self._feature_vector_np(selected_output, "atom_to_token_map"),
                    self._feature_vector_np(reference_output, "atom_to_token_map"),
                    self._feature_mask_np(
                        selected_output, "is_motif_atom_with_fixed_seq"
                    ),
                    self._feature_mask_np(
            ...
            selected_expected_tokens = (
                shared_token_indices_1_np
                if self.unconditional_reference_track == "track1"
                else shared_token_indices_2_np
            )
            if not np.array_equal(selected_token_check, selected_expected_tokens):
                raise ValueError(
                    "The isolated shared-chain reference does not preserve the "
                    "selected track's shared token ordering."
                )
        ...
        with torch.no_grad():
            ...
            reference_device = (
                self.trainer.fabric.to_device(reference_output)
            ...
            network_output = model.forward_coupled(
                ...
                reference_coord_atom_lvl_to_be_noised=(
                    reference_device["coord_atom_lvl_to_be_noised"]
                    if reference_device is not None
                    else None
                ),
                ...
                shared_update_token_indices_1=torch.as_tensor(
                    shared_token_indices_1_np,
                    dtype=torch.long,
                    device=device,
                ),
                shared_update_token_indices_2=torch.as_tensor(
                    shared_token_indices_2_np,
                    dtype=torch.long,
                    device=device,
                ),
                reference_update_atom_indices=(
                    torch.as_tensor(
                        reference_atom_map.update_indices_2,
                        dtype=torch.long,
                        device=device,
                    )
                    if reference_atom_map is not None
                    else None
                ),
                reference_kappa_atom_indices=(
                    torch.as_tensor(
                        reference_kappa_indices_np,
                        dtype=torch.long,
                        device=device,
                    )
                    if reference_kappa_indices_np is not None
                    else None
                ),
                reference_update_token_indices=(
                    torch.as_tensor(
                        reference_token_indices_np,
                        dtype=torch.long,
                        device=device,
                    )
                    if reference_token_indices_np is not None
                    else None
                ),
                coupling_metadata=self._base_coupling_metadata(
                    ...
                    int(shared_token_indices_1_np.size),
                ),
            )
    ...
    def _build_spec_pipeline_output(
        self,
        spec: DesignInputSpecification,
        *,
        example_id: str,
    ) -> tuple[dict, AtomArray, dict]:
        """Build a specification once and preserve its pre-pipeline atom array."""

        atom_array, spec_dict = spec.build(return_metadata=True)
        output = self._build_pipeline_output_from_atom_array(
            atom_array,
            spec_dict,
            example_id=example_id,
        )
        return output, atom_array, spec_dict

    def _build_pipeline_output_from_atom_array(
        self,
        atom_array: AtomArray,
        spec_dict: dict,
        *,
        ...
        """Create one transformed model input from an annotated atom array."""

        data = prepare_pipeline_input_from_atom_array(atom_array.copy())
        data["example_id"] = example_id
        saved_spec = deepcopy(spec_dict)
        saved_spec.setdefault("extra", {})["example_id"] = example_id
        data["specification"] = saved_spec
    ...
    def _build_coupled_track_specs(
        ...
        if getattr(base_spec, "symmetry", None) is not None:
            raise ValueError(
                "rfd3_system_v3 shared-chain coupling does not support symmetry."
            )
    ...
    @staticmethod
    def _feature_vector_np(pipeline_output: dict, feature_name: str) -> np.ndarray:
        """Return one unbatched atom-level feature vector as a NumPy array."""
        ...
        if vector.ndim != 1:
            raise ValueError(
                f"Expected atom-level feature {feature_name!r} to be 1D, "
    ...
    def _base_coupling_metadata(
        ...
        shared_sequence_token_count: int,
    ) -> dict:
        sequence_policy = (
            "Movable shared-chain sequence logits are mixed with the final "
            "applied coordinate kappa. Fixed or chemically variant motif "
            "residues retain track-specific identities. This is an output "
            "heuristic, not a sequence-density SuperDiff derivation."
        )
        return {
            ...
            "finite_step_ito_constraint": (
                "Algebraically exact before clamping for non-degenerate fields "
                "on the selected atom subset, up to floating-point error."
            ),
            "score_source": (
            ...
            "unconditional_reference_track": self.unconditional_reference_track,
            "unconditional_reference_evaluated": (
                float(
                    self.inference_sampler_overrides.get(
                        "superdiff_guidance_scale", 1.0
                    )
            ...
            "shared_sequence_token_count": int(shared_sequence_token_count),
