"""Chain-level helpers for shared-chain coupled sampling."""
...
def build_shared_update_atom_map(
    ...
    stochastic shared update and from the SuperDiff kappa solve. Non-fixed
    shared residues must still match exactly, preserving the original all-atom
    coupling behavior for ordinary generated chain A residues.
    """
...
def select_kappa_solve_atom_indices(
    ...
    for the stochastic density-change equation when requested.
    """
...
def shared_update_token_indices(
    shared_atom_map: SharedAtomMap,
    atom_to_token_map_1: np.ndarray,
    atom_to_token_map_2: np.ndarray,
    fixed_sequence_mask_1: np.ndarray,
    fixed_sequence_mask_2: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Return paired token indices for movable shared-chain residues.

    RFD3 emits sequence logits at token level while coordinate coupling uses
    atom indices.  Protein atoms belonging to one residue must map to exactly
    one token, and paired residues must remain in the same order across tracks.
    """

    token_map_1 = np.asarray(atom_to_token_map_1, dtype=np.int64)
    token_map_2 = np.asarray(atom_to_token_map_2, dtype=np.int64)
    fixed_sequence_mask_1 = np.asarray(fixed_sequence_mask_1, dtype=bool)
    fixed_sequence_mask_2 = np.asarray(fixed_sequence_mask_2, dtype=bool)
    if token_map_1.ndim != 1 or token_map_2.ndim != 1:
        raise ValueError("atom_to_token maps must be one-dimensional.")
    if fixed_sequence_mask_1.shape != token_map_1.shape:
        raise ValueError("Track 1 fixed-sequence mask must match its atom map.")
    if fixed_sequence_mask_2.shape != token_map_2.shape:
        raise ValueError("Track 2 fixed-sequence mask must match its atom map.")

    update_1 = np.asarray(shared_atom_map.update_indices_1, dtype=np.int64)
    update_2 = np.asarray(shared_atom_map.update_indices_2, dtype=np.int64)
    if update_1.shape != update_2.shape:
        raise ValueError("Shared update atom index arrays must have matching shapes.")
    if update_1.size == 0:
        raise ValueError("Shared update atom map cannot be empty.")
    if update_1.max() >= token_map_1.size or update_2.max() >= token_map_2.size:
        raise ValueError("Shared atom index exceeds its atom-to-token map.")

    all_tokens_1 = _ordered_unique(token_map_1[update_1])
    all_tokens_2 = _ordered_unique(token_map_2[update_2])
    expected_residues = len(shared_atom_map.update_residues)
    if (
        all_tokens_1.size != expected_residues
        or all_tokens_2.size != expected_residues
    ):
        raise ValueError(
            "Movable shared residues do not map one-to-one onto sequence tokens: "
            f"expected {expected_residues}, got "
            f"{all_tokens_1.size} and {all_tokens_2.size}."
        )

    keep = []
    for token_1, token_2 in zip(all_tokens_1, all_tokens_2):
        fixed_1 = bool(np.any(fixed_sequence_mask_1[token_map_1 == token_1]))
        fixed_2 = bool(np.any(fixed_sequence_mask_2[token_map_2 == token_2]))
        keep.append(not fixed_1 and not fixed_2)
    keep = np.asarray(keep, dtype=bool)
    return all_tokens_1[keep], all_tokens_2[keep]


def _ordered_unique(values: np.ndarray) -> np.ndarray:
    """Return unique integer values while preserving first appearance."""
