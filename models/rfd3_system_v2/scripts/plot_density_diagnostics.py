#!/usr/bin/env python3
"""Plot rfd3_system_v2 deterministic density-coupling diagnostics."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Create PNG plots for rfd3_system_v2 kappa, density residual, and "
            "field/path cosine diagnostics after inference."
        ),
    )
    parser.add_argument(
        "paths",
        nargs="+",
        type=Path,
        help="Output directories or specific rfd3_system_v2 JSON metadata files.",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
...
def _collect_diagnostic_sources(
    ...
    rfd3_system_v2 writes identical diagnostics into track and merged JSON
...
def _plot_one_source(
    ...
    *,
    ...
    """Create density-control diagnostic PNGs for one diffusion batch."""
    ...
    if "density_rate_residual" in diagnostics:
        residual, labels = _diagnostic_array_for_model(
            diagnostics["density_rate_residual"],
        ...
        residual_path = plot_prefix.with_name(
            f"{plot_prefix.name}_density_rate_residual.png"
        )
    ...
    if "validation_density_rate_residual" in diagnostics:
        residual, labels = _diagnostic_array_for_model(
            diagnostics["validation_density_rate_residual"],
        ...
        if np.isfinite(residual).any():
            ...
            residual_path = plot_prefix.with_name(
                f"{plot_prefix.name}_validation_density_rate_residual.png"
            )
            _plot_density_residual(
                residual_path,
                normalized_t=normalized_t,
    ...
    cosine_specs = (
        (
            ...
            "Kappa-subset conditional-field/path cosine similarity",
        ),
        (
            "all_shared",
            "cosine_v1_path_all_shared",
            "cosine_v2_path_all_shared",
            "All-shared-atom conditional-field/path cosine similarity",
        ),
    )
...
def _plot_density_residual(
    ...
    *,
    ...
    """Plot post-clamp density-rate residual trajectories."""

    fig, ax = plt.subplots(figsize=(8.5, 5.0), constrained_layout=True)
    _plot_samples(ax, normalized_t, residual, sample_labels)
    ax.axhline(0.0, color="0.35", linestyle="--", linewidth=1.0)
    title_prefix = "Independent-probe validation" if validation else "Solve-probe"
    ax.set_title(f"{title_prefix} density-rate residual over denoising")
    ax.set_xlabel("Normalized denoising progress t (0 = noisiest, 1 = final)")
    ax.set_ylabel("rho_1 - rho_2 - target (after clamp)")
    ax.set_xlim(0.0, 1.0)
    ...
    ax.text(
        ...
        -0.22,
        (
            "Residual near zero means the estimated conditional density-rate "
    ...
    *,
    ...
    for sample_idx in range(n_plot):
        ...
        ax.plot(
            ...
            color=light_color,
            linewidth=1.8,
            label=f"{label}: cos(v1, applied path)",
        )
        ax.plot(
            normalized_t,
            cosine_2[:, sample_idx],
            color=dark_color,
            linewidth=1.8,
            label=f"{label}: cos(v2, applied path)",
        )
    ...
    note = (
        f"Light shade: cos(v1 from {track_1_label}, applied path). "
        f"Dark shade: cos(v2 from {track_2_label}, applied path). "
