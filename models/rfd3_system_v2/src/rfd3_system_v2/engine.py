...
from rfd3_system_v2.constants import SAVED_CONDITIONING_ANNOTATIONS
from rfd3_system_v2.inference.datasets import (
    assemble_distributed_inference_loader_from_json,
)
from rfd3_system_v2.inference.input_parsing import (
    DesignInputSpecification,
    ensure_input_is_abspath,
    prepare_pipeline_input_from_atom_array,
)
from rfd3_system_v2.model.inference_sampler import SampleDiffusionConfig
from rfd3_system_v2.system.chains import (
...
from rfd3_system_v2.utils.inference import (
    ensure_inference_sampler_matches_design_spec,
)
from rfd3_system_v2.utils.io import (
    CIF_LIKE_EXTENSIONS,
...
class RFD3InferenceConfig:
    ...
    # Deterministic density-controlled shared-chain prototype. Off by default.
...
class RFD3InferenceEngine(BaseInferenceEngine):
    ...
    def _override_checkpoint_config(self, cfg):
        """Load the RFD3 checkpoint config through the rfd3_system_v2 package.

        The public RFD3 checkpoint stores Hydra targets under the original
        `rfd3.*` package. This research copy must instantiate local
        `rfd3_system_v2.*` classes so the coupled sampler registry and engine
        ...
        if isinstance(node, DictConfig):
            if "_target_" in node and isinstance(node["_target_"], str):
                ...
                if target.startswith("rfd3."):
                    node["_target_"] = "rfd3_system_v2." + target[len("rfd3.") :]
    ...
    def run(
        ...
        *,
        ...
        if len(design_specifications) == 0:
            ...
            return None
        if self.coupling_mode == "superdiff_shared_chain_density":
            if (
                self.inference_sampler_overrides.get("kind")
                != "superdiff_shared_chain_density"
            ):
                raise ValueError(
                    "coupling_mode='superdiff_shared_chain_density' requires "
                    "inference_sampler.kind='superdiff_shared_chain_density'."
                )
            if self.unconditional_reference_track not in ("track1", "track2"):
                raise ValueError(
                    "unconditional_reference_track must be explicitly set to "
                    "'track1' or 'track2'."
                )
            # init before
            self.initialize()
    ...
    def _run_superdiff_shared_chain(
        ...
        """Run the deterministic density-controlled A+B/A+C prototype."""

        if self.trainer.fabric.world_size != 1:
            raise ValueError(
                "Density-controlled shared-chain coupling supports one process. "
    ...
    def _base_coupling_metadata(
        ...
        return {
            ...
            "implementation": "post-churn deterministic density-rate estimate",
