"""Adapted BindCraft BioPython utilities for rfd3_system.

Adapted from BindCraft ``functions/biopython_utils.py`` at commit
``b971db42ba6e091afab63ccb30ae02215150a990``.

The active functions preserve BindCraft-style calculations while accepting
explicit target/binder chain IDs where the original utilities assumed target A
and binder B.
"""
...
# DROPPED: BindCraft-specific sequence validation.
...
def target_pdb_rmsd(
    ...
    # Extract the selected chain from trajectory_pdb
    chain_trajectory = structure_trajectory[0][trajectory_chain_id]
    ...
    # Extract residues from selected chain in trajectory_pdb
