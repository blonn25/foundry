...
from rfd3_system_v3.model.cfg_utils import (
    strip_f,
)
from rfd3_system_v3.model.inference_sampler import ConditionalDiffusionSampler
from rfd3_system_v3.model.layers.encoders import TokenInitializer
...
class RFD3(nn.Module):
    ...
    def forward_coupled(
        ...
        *,
        ...
        """Run stochastic reverse-SDE SuperDiff inference for two tracks.

        This inference-only path uses the public RFD3 denoiser and delegates the
        full trajectory to the `superdiff_shared_chain_sde` sampler.
        """

        if self.training:
            raise ValueError("forward_coupled is only valid during inference.")
        if self.use_classifier_free_guidance:
            raise ValueError(
                "Stochastic shared-chain coupling does not support native "
                "classifier-free guidance."
            )

        initializer_outputs_1 = self.token_initializer(track_1_input["f"])
        initializer_outputs_2 = self.token_initializer(track_2_input["f"])
        initializer_outputs_0 = (
            self.token_initializer(reference_input["f"])
            if reference_input is not None
            else None
        )

        return self.inference_sampler.sample_coupled_superdiff_sde(
            track_1={
                "f": track_1_input["f"],
                "coord_atom_lvl_to_be_noised": track_1_coord_atom_lvl_to_be_noised,
            ...
            reference=(
                {
                    ...
                    "initializer_outputs": initializer_outputs_0,
                }
                if reference_input is not None
                else None
            ),
            ...
            shared_kappa_atom_indices_1=shared_kappa_atom_indices_1,
            shared_kappa_atom_indices_2=shared_kappa_atom_indices_2,
            shared_update_token_indices_1=shared_update_token_indices_1,
            shared_update_token_indices_2=shared_update_token_indices_2,
            reference_update_atom_indices=reference_update_atom_indices,
            reference_kappa_atom_indices=reference_kappa_atom_indices,
            reference_update_token_indices=reference_update_token_indices,
            diffusion_module=self.diffusion_module,
