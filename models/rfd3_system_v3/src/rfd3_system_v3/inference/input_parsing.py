...
def _selection_requires_atom_array(selection_name: str, value: Any) -> bool:
    """Return whether a selection needs source atoms during validation.

    A pure de novo contig such as "90,/0,80,/0,100" only describes generated
    residue lengths and chain breaks, so it can be expanded without an input
    atom array.  Any chain-letter reference, e.g. "A1-90", still requires an
    input atom array because it selects source residues.
    """

    if selection_name != "contig":
        return True
    raw = getattr(value, "raw", value)
    return bool(re.search(r"[A-Za-z]", str(raw)))
...
class DesignInputSpecification(BaseModel):
    ...
    def validate_input_schema(cls, data: dict) -> dict:
        if not (
            ...
            or exists(data.get("atom_array_input"))
            or exists(data.get("contig"))
            or exists(data.get("length"))
        ):
            raise ValueError(
                "Either 'input', 'atom_array_input', or 'contig' / 'length' must be provided."
            )
        ...
        with validator_context("load_input"):
            ...
            if not exists(data.get("input")) and not exists(
                ...
                source_backed_selections = [
                    ...
                    if _selection_requires_atom_array(selection, data.get(selection))
                ]
                if source_backed_selections:
                    raise ValueError(
